...
def plot_ray_density(A, g):
    ...
    plt.imshow(ray_density.reshape(g.npoints).T, extent=[g.origin[0],
...
def plot_rays(sources, receivers, g, only_locations=False):
    ...
    plt.scatter(sources[0,:], sources[1,:], marker='*')
    plt.scatter(receivers[0,:], receivers[1,:], marker='v', color='g')
